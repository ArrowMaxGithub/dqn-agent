...
class LowCardAgent(AgentInterface):
    def __init__(
        self,
        passing_action,
    ):
...
class LowCardRLModule(InferenceOnlyAPI, TorchRLModule):
    def setup(self):
...
def test():
    env = DurakEnv()
    agents_dict = {
        agent_id: LowCardAgent(env.passing_action) for agent_id in env.possible_agents
    }
